...
uint8_t stockDeckPropertiesPrimaryFcn()
{
  return stockDeckPropertiesPrimary;
}

uint8_t stockDeckPropertiesSecondary;
uint8_t stockDeckPropertiesSecondaryFcn()
{
  return stockDeckPropertiesSecondary;
}

uint32_t read_vAddr;
uint8_t read_len;
bool read_isCalled;
bool mockRead(const uint32_t vAddr, const uint8_t len, uint8_t *buffer)
{
  read_isCalled = true;
  read_vAddr = vAddr;
  read_len = len;
...
bool mockWrite(const uint32_t vAddr, const uint8_t len, const uint8_t *buffer,
...
void testOneDeckUpgradePrimarySupported()
{
  ...
  // Assert
  TEST_ASSERT_EQUAL_UINT8(MASK_IS_VALID | MASK_SUPPORTS_WRITE | MASK_SUPPORTS_UPGRADE,
                          buffer[DECK_0_INFO_PRIMARY]);
}
