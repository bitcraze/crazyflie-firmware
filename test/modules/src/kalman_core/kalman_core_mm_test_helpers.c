...
void mock_kalmanCoreScalarUpdate_callback(kalmanCoreData_t *actualThis,
    arm_matrix_instance_f32 *actualHm, float actualError, float actualStdMeasNoise,
    int cmock_num_calls);
...
void setKalmanCoreScalarUpdateExpectationsSingleCall(const kalmanCoreData_t *expectedThis,
    const float *expectedHm, const float expectedError, const float expectedStdMeasNoise)
{
    ...
    int cmock_num_calls)
{
  ...
  TEST_ASSERT_EQUAL_UINT16_MESSAGE(KC_STATE_DIM, actualHm->numCols,
                                   "Unexpected nr of collumns in Hm vector");
  for (int i = 0; i < KC_STATE_DIM; i++) {
    TEST_ASSERT_FLOAT_WITHIN_MESSAGE(1e-20, kcsus_expectedHm[i], actualHm->pData[i],
                                     "Unexpected value of element in Hm vector");
  }

  TEST_ASSERT_EQUAL_FLOAT_MESSAGE(kcsus_expectedError, actualError, "Unexpected value of error");
  TEST_ASSERT_EQUAL_FLOAT_MESSAGE(kcsus_expectedStdMeasNoise, actualStdMeasNoise,
                                  "Unexpected value of stdMeasNoise");
}
...
{
  TEST_ASSERT_FALSE_MESSAGE(kcsus_scalarUpadateWasCalled,
                            "kalmanCoreScalarUpdate() got an unexpected call");
}
