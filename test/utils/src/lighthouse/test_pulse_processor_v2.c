...
int processFrame(const pulseProcessorFrame_t *frameData,
                 pulseProcessorV2PulseWorkspace_t *pulseWorkspace, pulseProcessorV2BlockWorkspace_t *blockWorkspace);
bool storePulse(const pulseProcessorFrame_t *frameData,
...
bool processWorkspaceBlock(const pulseProcessorFrame_t slots[],
                           pulseProcessorV2SweepBlock_t *block);
bool isBlockPairGood(const pulseProcessorV2SweepBlock_t *latest,
                     const pulseProcessorV2SweepBlock_t *storage);
...
static void setUpOkBlocks(pulseProcessorV2SweepBlock_t *newBlock,
                          pulseProcessorV2SweepBlock_t *storageBlock);
static void addFrameToWs(uint8_t sensor, uint32_t timestamp, uint32_t offset, bool channelFound,
                         uint8_t channel);
static void setUpOotxDecoderProcessBitCallCounter();
static void setUpSlowbitFrame();
static void clearSlowbitState();
static void validateProcessFrame(int expectedNrOfBlocks, uint8_t sensor, uint32_t timestamp,
                                 uint32_t offset, uint8_t channel, bool channelFound);
