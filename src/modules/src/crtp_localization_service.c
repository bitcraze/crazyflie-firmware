...
static void extPosePackedHandler(const CRTPPacket *pk)
{
  ...
  for (uint8_t i = 0; i < numItems; ++i) {
    const extPosePackedItem *item = (const extPosePackedItem *)&pk->data[1 + i * sizeof(
                                        extPosePackedItem)];
    if (item->id == my_id) {
...
static void extPositionPackedHandler(CRTPPacket *pk)
{
  ...
  for (uint8_t i = 0; i < numItems; ++i) {
    const extPositionPackedItem *item = (const extPositionPackedItem *)&pk->data[i * sizeof(
                                            extPositionPackedItem)];
    ext_pos.x = item->x / 1000.0f;
    ext_pos.y = item->y / 1000.0f;
    ext_pos.z = item->z / 1000.0f;
    ext_pos.stdDev = extPosStdDev;
...
void locSrvSendLighthouseAngle(int baseStation, pulseProcessorResult_t *angles)
{
  ...
  if (enableLighthouseAngleStream) {
    ...
    pulseProcessorBaseStationMeasurement_t *baseStationMeasurement =
      &angles->baseStationMeasurementsLh1[baseStation];
