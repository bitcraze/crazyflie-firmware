...
static inline void filter_ddxi(Butterworth2LowPass *filter, struct Vectr *old_values,
                               struct Vectr *new_values)
{
...
// Attitude filter
static inline void filter_ang(Butterworth2LowPass *filter, struct Angles *old_values,
                              struct Angles *new_values)
{
                            ...
                            vector_t *refOuterINDI)
{
  ...
  indiOuter.linear_accel_ft.x = M_OB[0][0] * indiOuter.linear_accel_f.x + M_OB[0][1] *
                                indiOuter.linear_accel_f.y + M_OB[0][2] * indiOuter.linear_accel_f.z;
  indiOuter.linear_accel_ft.y = M_OB[1][0] * indiOuter.linear_accel_f.x + M_OB[1][1] *
                                indiOuter.linear_accel_f.y + M_OB[1][2] * indiOuter.linear_accel_f.z;
  indiOuter.linear_accel_ft.z = M_OB[2][0] * indiOuter.linear_accel_f.x + M_OB[2][1] *
                                indiOuter.linear_accel_f.y + M_OB[2][2] * indiOuter.linear_accel_f.z + 9.81f;
  ...
  // Determinant of (G'*G)
  float detG = (a11 * a22 * a33 + a12 * a23 * a31 + a21 * a32 * a13) - (a13 * a22 * a31 + a11 * a32 *
               a23 + a12 * a21 * a33);
  ...
  indiOuter.phi_tilde   = (g11_inv * indiOuter.linear_accel_err.x + g12_inv *
                           indiOuter.linear_accel_err.y + g13_inv * indiOuter.linear_accel_err.z);
  indiOuter.theta_tilde = (g21_inv * indiOuter.linear_accel_err.x + g22_inv *
                           indiOuter.linear_accel_err.y + g23_inv * indiOuter.linear_accel_err.z);
  indiOuter.T_tilde     = -(g31_inv * indiOuter.linear_accel_err.x + g32_inv *
                            indiOuter.linear_accel_err.y + g33_inv * indiOuter.linear_accel_err.z) / K_thr;
  ...
  indiOuter.T_inner = indiOuter.T_inner + indiOuter.act_dyn_posINDI * (indiOuter.T_inner_f -
                      indiOuter.T_inner);
  ...
  indiOuter.attitude_c.theta = clamp(indiOuter.attitude_c.theta, -radians(pq_clamp),
                                     radians(pq_clamp));
