                    ...
                    const stabilizerStep_t stabilizerStep)
{
  ...
  if (RATE_DO_EXECUTE(ATTITUDE_RATE, stabilizerStep)) {
    ...
    if (setpoint->mode.yaw == modeVelocity) {
      attitudeDesired.yaw += setpoint->attitudeRate.yaw *
                             ATTITUDE_UPDATE_DT; //if line 140 (or the other setpoints) in crtp_commander_generic.c has the - sign remove add a -sign here to convert the crazyfly coords (ENU) to INDI  body coords (NED)
      ...
      attitudeDesired.yaw = capAngle(
                              attitudeDesired.yaw); //use the capangle as this is also done in velocity mode
      attitudeDesired.yaw = -radians(
                              attitudeDesired.yaw); //convert to radians and add negative sign to convert from ENU to NED
    }
  }
  ...
  if (RATE_DO_EXECUTE(ATTITUDE_RATE, stabilizerStep)) {
    ...
    if (setpoint->mode.x == modeDisable) {
      ...
      // INDI position controller not active, INDI attitude controller is main loop
      attitudeDesired.roll = radians(
                               setpoint->attitude.roll); //no sign conversion as CF coords is equal to NED for roll
    ...
    if (setpoint->mode.y == modeDisable) {
      ...
      // INDI position controller not active, INDI attitude controller is main loop
      attitudeDesired.pitch = radians(
                                setpoint->attitude.pitch); //no sign conversion as CF coords use left hand for positive pitch.
    ...
    rateDesired.roll 	= indi.reference_acceleration.err_p * (attitudeDesired.roll - radians(
                          state->attitude.roll));
    rateDesired.pitch 	= indi.reference_acceleration.err_q * (attitudeDesired.pitch - radians(
                            state->attitude.pitch));
    rateDesired.yaw 	= indi.reference_acceleration.err_r * (attitudeDesired.yaw - (-radians(
                          state->attitude.yaw))); //negative yaw ENU  ->  NED
    ...
    body_rates.q = -radians(
                     sensors->gyro.y); //Account for gyro measuring pitch rate in opposite direction relative to both the CF coords and INDI coords
    ...
    indi.du.r = 1.0f / (indi.g1.r + indi.g2) * (indi.angular_accel_ref.r - indi.rate_d[2] + indi.g2 *
                indi.du.r);
