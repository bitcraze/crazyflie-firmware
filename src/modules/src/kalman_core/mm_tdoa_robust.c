                                    ...
                                    OutlierFilterTdoaState_t *outlierFilterState)
{
  ...
  if ((d0 != 0.0f) && (d1 != 0.0f)) {
    ...
    memcpy(X_state, this->S, sizeof(
             X_state));                     // copy Xpr to X_State and then update in each iterations
    ...
    for (int iter = 0; iter < MAX_ITER; iter++) {
      ...
      float predicted_iter = d1 -
                             d0;                           // predicted measurements in each iteration based on X_state
      float error_iter = measurement -
                         predicted_iter;          // innovation term based on iterated X_state
      ...
      if ((d0 != 0.0f) && (d1 != 0.0f)) {
        ...
        mat_mult(&Pc_w_invm, &Pc_tran_m,
                 &P_w_m);             // P_w_m = Pc_w_invm.dot(Pc_tran_m) = P_chol.dot(linalg.inv(w_x)).dot(P_chol.T)
        // rescale R matrix
        float w_y = 0.0;
        float R_w = 0.0f;
        ...
        mat_trans(&H, &HTm);
        mat_mult(&P_w_m, &HTm,
                 &PHTm);                        // PHTm = P_w.dot(H.T). The P is the updated P_w

        float HPHR =
          R_w;                                     // HPH' + R.            The R is the updated R_w
        for (int i = 0; i < KC_STATE_DIM; i++) {              // Add the element of HPH' to the above
          HPHR += h[i] *
                  PHTd[i];                           // this only works if the update is scalar (as in this function)
        }
        // ====== MEASUREMENT UPDATE ======
        // Calculate the Kalman gain and perform the state update
        for (int i = 0; i < KC_STATE_DIM; i++) {
          Kw[i] = PHTd[i] /
                  HPHR;                           // rescaled kalman gain = (PH' (HPH' + R )^-1) with the updated P_w and R_w
