...
static void extRxDecodeChannels(void)
{
...
#if CONFIG_DECK_EXTRX_ARMING
  if (EXTRX_SIGN_ARM * cppmConvert2Float(ch[EXTRX_CH_ARM], -1, 1,
                                         0.0) > 0.5f) { // channel needs to be 75% or more to work correctly with 2/3 way switches
    if (arm_cnt < EXTRX_SWITCH_MIN_CNT) {
      arm_cnt++;
    } else {
      extRxArm = true;
    }
  ...
  if (EXTRX_SIGN_ALTHOLD * cppmConvert2Float(ch[EXTRX_CH_ALTHOLD], -1, 1, 0.0) > 0.5f) {
    if (altHold_cnt < EXTRX_SWITCH_MIN_CNT) {
      altHold_cnt++;
    } else {
      extRxAltHold = true;
    }

    if (extRxAltHoldPrev != extRxAltHold) {
      DEBUG_PRINT("Althold mode ON\n");
    }
  } else {
    if (altHold_cnt > 0) {
      altHold_cnt--;
...
#if CONFIG_DECK_EXTRX_MODE_RATE
  if (EXTRX_SIGN_MODE * cppmConvert2Float(ch[EXTRX_CH_MODE], -1, 1,
                                          0.0) > 0.5f) { // channel needs to be 75% or more to work correctly with 2/3 way switches
    if (modeRate_cnt < EXTRX_SWITCH_MIN_CNT) {
      modeRate_cnt++;
    } else {
      extRxModeRate = true;
    }
  ...
  if (extRxModeRate) {
    extrxSetpoint.attitudeRate.roll = EXTRX_SIGN_ROLL * getCPPMRollRateScale() * cppmConvert2Float(
                                        ch[EXTRX_CH_ROLL], -1, 1, EXTRX_DEADBAND_ROLL);
    extrxSetpoint.attitudeRate.pitch = EXTRX_SIGN_PITCH * getCPPMPitchRateScale() * cppmConvert2Float(
                                         ch[EXTRX_CH_PITCH], -1, 1, EXTRX_DEADBAND_PITCH);
  } else {
    extrxSetpoint.attitude.roll = EXTRX_SIGN_ROLL * getCPPMRollScale() * cppmConvert2Float(
                                    ch[EXTRX_CH_ROLL], -1, 1, EXTRX_DEADBAND_ROLL);
    extrxSetpoint.attitude.pitch = EXTRX_SIGN_PITCH * getCPPMPitchScale() * cppmConvert2Float(
                                     ch[EXTRX_CH_PITCH], -1, 1, EXTRX_DEADBAND_PITCH);
  }
  extrxSetpoint.attitudeRate.yaw = EXTRX_SIGN_YAW * getCPPMYawRateScale() * cppmConvert2Float(
                                     ch[EXTRX_CH_YAW], -1, 1, EXTRX_DEADBAND_YAW);
