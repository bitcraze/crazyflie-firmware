...
// In unit test builds there is no .eventtrigger linker section, but the
// payload and trigger definitions are still needed so that files using
// EVENTTRIGGER can be compiled into a test.
#ifndef UNIT_TEST_MODE
#define _EVENTTRIGGER_ATTRIBUTES(NAME) __attribute__((section(".eventtrigger." #NAME), used))
#else
#define _EVENTTRIGGER_ATTRIBUTES(NAME) __attribute__((used))
#endif
