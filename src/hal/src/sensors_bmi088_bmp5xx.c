 ...
 *
 * sensors_bmi088_bmp5xx.c: IMU sensor driver for the bmi088 + bmp5xx sensors
 */
...
#define SENSORS_READ_BARO_HZ            45
...
static struct bmp5_dev   bmp5xxDev;
static uint8_t bmp5_i2c_addr = BMP5_I2C_ADDR_SEC;
static struct bmp5_osr_odr_press_config bmp5_osr_odr_cfg;
...
// BMP5 callback wrappers (different signature from BMI088/BMP3)

static BMP5_INTF_RET_TYPE bmp5_i2c_read(uint8_t reg_addr, uint8_t *data, uint32_t len, void *intf_ptr)
{
  uint8_t dev_addr = *(uint8_t *)intf_ptr;
  if (i2cdevReadReg8(I2C3_DEV, dev_addr, reg_addr, (uint16_t)len, data))
  {
...
static BMP5_INTF_RET_TYPE bmp5_i2c_write(uint8_t reg_addr, const uint8_t *data, uint32_t len, void *intf_ptr)
{
  uint8_t dev_addr = *(uint8_t *)intf_ptr;
  if (i2cdevWriteReg8(I2C3_DEV, dev_addr, reg_addr, (uint16_t)len, (uint8_t *)data))
  {
    return BMP5_INTF_RET_SUCCESS;
  }
  else
...
static void bmp5_delay_us_cb(uint32_t period, void *intf_ptr)
{
  (void)intf_ptr;
                             ...
                             float temperature)
{
  ...
  baroScaled->asl = ((powf((101570.0f / baroScaled->pressure), 0.1902630958f)
...
bool sensorsBmi088Bmp5xxAreCalibrated()
{
  ...
  while (1)
  {
    ...
    if (isBarometerPresent)
    {
      ...
      if (--baroMeasDelay == 0)
      {
        ...
        baro_t* baro581 = &sensorData.baro;
        bmp5_get_sensor_data(&data, &bmp5_osr_odr_cfg, &bmp5xxDev);
        sensorsScaleBaro(baro581, data.pressure, data.temperature);
...
static void sensorsDeviceInit(void)
{
  ...
  /* BMP581 */
  bmp5xxDev.intf_ptr = &bmp5_i2c_addr;
  bmp5xxDev.intf = BMP5_I2C_INTF;
  bmp5xxDev.read = bmp5_i2c_read;
  bmp5xxDev.write = bmp5_i2c_write;
  bmp5xxDev.delay_us = bmp5_delay_us_cb;

  int8_t bmp5_rslt;
  int i = 3;
  do {
    bmp5_delay_us_cb(1000, NULL);
    bmp5_rslt = bmp5_init(&bmp5xxDev);
  } while (bmp5_rslt != BMP5_OK && i-- > 0);

  if (bmp5_rslt == BMP5_OK)
  {
    isBarometerPresent = true;
    DEBUG_PRINT("BMP581 I2C connection [OK]\n");

    /* Set standby mode before configuring */
    bmp5_rslt = bmp5_set_power_mode(BMP5_POWERMODE_STANDBY, &bmp5xxDev);

    /* Configure OSR and ODR */
    bmp5_osr_odr_cfg.osr_t = BMP5_OVERSAMPLING_1X;
    bmp5_osr_odr_cfg.osr_p = BMP5_OVERSAMPLING_32X;
    bmp5_osr_odr_cfg.press_en = BMP5_ENABLE;
    bmp5_osr_odr_cfg.odr = BMP5_ODR_45_HZ;
    bmp5_rslt = bmp5_set_osr_odr_press_config(&bmp5_osr_odr_cfg, &bmp5xxDev);

    /* Configure IIR filter */
    struct bmp5_iir_config iir_cfg = {0};
    iir_cfg.set_iir_t = BMP5_IIR_FILTER_COEFF_3;
    iir_cfg.set_iir_p = BMP5_IIR_FILTER_COEFF_3;
    iir_cfg.shdw_set_iir_t = BMP5_ENABLE;
    iir_cfg.shdw_set_iir_p = BMP5_ENABLE;
    bmp5_rslt = bmp5_set_iir_config(&iir_cfg, &bmp5xxDev);

    /* Set normal mode */
    bmp5_rslt = bmp5_set_power_mode(BMP5_POWERMODE_NORMAL, &bmp5xxDev);

    bmp5_delay_us_cb(20000, NULL); // wait before first read out

    /* Read out data to verify */
    struct bmp5_sensor_data data;
    bmp5_rslt = bmp5_get_sensor_data(&data, &bmp5_osr_odr_cfg, &bmp5xxDev);
  ...
  else
  {
    ...
    DEBUG_PRINT("BMP5XX I2C connection [FAIL]\n");
    isInit = false;
    return;
#endif
  }
...
static void sensorsBmi088Bmp5xxInit(void)
{
...
/**
 * @brief Nonzero if BMP5XX barometer is present
 */
PARAM_ADD(PARAM_UINT8 | PARAM_RONLY, BMP5XX, &isBarometerPresent)
