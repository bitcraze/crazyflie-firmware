...
static void sensorsAccAlignToGravity(Axis3i16* in, Axis3i16* out);
...
static void sensorsRead(Axis3f* gyroOut, Axis3f* accOut)
{
  ...
  case SENSORS_BMI160:
	...
	#ifdef SENSORS_TAKE_ACCEL_BIAS
	  if (gyroBiasBmi160.isBiasValueFound &&
	      !accelBiasBmi160.isBiasValueFound)
	  {
	    Axis3i32 mean;
...
#ifdef SENSORS_ENABLE_BMI160
	  sensorsAccIIRLPFilter(&accelBmi160, &accelLPF, &accelStoredFilterValues,
	  ...
	  accOut->x = (accelLPF.x) * SENSORS_G_PER_LSB_CFG;
	  accOut->y = (accelLPF.y) * SENSORS_G_PER_LSB_CFG;
	  accOut->z = (accelLPF.z) * SENSORS_G_PER_LSB_CFG;
#endif
	  break;
  case SENSORS_BMI055:
	  sensorsAddBiasValue(&gyroBiasBmi055, &gyroBmi055);
	  	...
	  	#ifdef SENSORS_TAKE_ACCEL_BIAS
	  	  if (gyroBiasBmi055.isBiasValueFound &&
	  	      !accelBiasBmi055.isBiasValueFound)
	  	  {
	  	    Axis3i32 mean;
	  ...
	  accOut->x = (accelLPF.x - accelBiasBmi055.bias.x) * SENSORS_BMI055_1G_PER_LSB_CFG;
	  accOut->y = (accelLPF.y - accelBiasBmi055.bias.y) * SENSORS_BMI055_1G_PER_LSB_CFG;
	  accOut->z = (accelLPF.z - accelBiasBmi055.bias.z) * SENSORS_BMI055_1G_PER_LSB_CFG;
#endif
	  break;
  }
}
