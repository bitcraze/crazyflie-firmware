/** 
 * @file visual_slam.c   
 * @brief Fly around autonomously, take pics with the AI deck and distances with the multiranger
 *
 * Uses the app layer in the crazyflie firmware.
 * 
 * Add regular checking of battery level and land accordingly
 * Add a* algo to search around for some picture input
 *
 */
...
void appMain()
{
  ...
  uint16_t idDown = logGetVarId("range", "zrange");

  uint16_t idAiByte = logGetVarId("aideck", "receivebyte");
  ...
  while(1) {
    ...
    if (state == unlocked) {
      ...
      uint8_t visual_signal = logGetUint(idAiByte);
      ...
      if (1) {
        setHoverSetpoint(&setpoint, velFront, velSide, height, 0);   /*this way we only hover until disturbed by hand */
        commanderSetSetpoint(&setpoint, 3);   /* we should take some sensor input (/e.g AI deck byte) and start a search algorithm */
      }
