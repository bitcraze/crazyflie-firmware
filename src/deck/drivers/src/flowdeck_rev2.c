...
static void pamotionTask(void *param)
{
  ...
  while(1) {
    ...
    if (abs(accpx) < OULIER_LIMIT && abs(accpy) < OULIER_LIMIT) {
      ...
      if (!useFlowDisabled) {
        DEBUG_PRINT("raw %f %f\n", (double)flowData.dpixelx, (double)flowData.dpixely);
        estimatorEnqueueFlow(&flowData);
      }
    } else {
      outlierCount++;
    }
    // Register manipulation hack
    if (execute)
    {
      registerWrite(regToWrite, valToWrite);
      execute = 0;
      DEBUG_PRINT("Reg after write: 0x%x:0x%x\n", regToWrite, registerRead(regToWrite));
    }
  }
}
...
PARAM_ADD(PARAM_UINT8, reg, &regToWrite)
PARAM_ADD(PARAM_UINT8, value, &valToWrite)
PARAM_ADD(PARAM_UINT8, execute, &execute)
