...
static uint16_t parser_service_payload_limit(const bccam_uart_frame_parser_t *parser) {
    if (parser->service == BCCAM_UART_SERVICE_LINK_MANAGEMENT) {
        return BCCAM_UART_MANAGEMENT_MAX_PAYLOAD;
    }

    return parser->max_payload_len;
}
...
int bccam_uart_frame_parser_feed(bccam_uart_frame_parser_t *parser,
    ...
    *out_frame_ready = false;
    ...
    case BCCAM_UART_PARSE_VERSION:
        ...
        if (byte != BCCAM_UART_FRAME_VERSION) {
    ...
    case BCCAM_UART_PARSE_LEN1:
        ...
        parser->header_index = 4;
        parser->payload_len |= (uint16_t)byte << 8;
        const uint16_t max_payload_len = parser_service_payload_limit(parser);
