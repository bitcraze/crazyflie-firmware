...
#define BCCAM_UART_RX_FRAME_POOL_LENGTH 4
...
#define BCCAM_CONTROL_SERVICE_MAJOR 1u
#define BCCAM_CONTROL_SERVICE_MINOR 0u
...
typedef enum {
  BCCAM_UART_MODE_REQUEST_NONE,
  BCCAM_UART_MODE_REQUEST_ENTER_BOOTLOADER,
  BCCAM_UART_MODE_REQUEST_RESET_TO_FIRMWARE,
} bccam_uart_mode_request_t;
...
static volatile bccam_uart_mode_request_t pending_mode_request =
  BCCAM_UART_MODE_REQUEST_NONE;
...
static bccam_uart_service_entry_t
  test_firmware_advertised_services[BCCAM_UART_MAX_DISCOVERED_SERVICES];
static uint8_t test_firmware_advertised_service_count;
...
} bccam_uart_rx_frame_slot_t;

// Intentionally ordinary RAM: frame storage may need DMA access in production.
static bccam_uart_rx_frame_slot_t
  rx_frame_pool[BCCAM_UART_RX_FRAME_POOL_LENGTH];
...
static void observe_firmware_client(
  ...
  if (observation != NULL && test_firmware_advertised_service_count > 0) {
    observation->advertised_service_count =
      test_firmware_advertised_service_count;
  }
#endif
}

static const bccam_uart_service_entry_t *advertised_service_at(uint8_t index) {
#if defined(UNIT_TEST) || defined(UNIT_TEST_MODE)
  if (index < test_firmware_advertised_service_count) {
    return &test_firmware_advertised_services[index];
  }
#endif

  return bccam_firmware_uart_client_advertised_service(&firmware_client, index);
}
...
static bool is_firmware_state(bccam_uart_service_state_t state) {
  return state == BCCAM_UART_SERVICE_STATE_FW_RESETTING ||
         state == BCCAM_UART_SERVICE_STATE_FW_DISCOVERY ||
...
static bool is_rx_firmware_state(bccam_uart_service_state_t state) {
  return state == BCCAM_UART_SERVICE_STATE_FW_DISCOVERY ||
         state == BCCAM_UART_SERVICE_STATE_FW_ACTIVE;
}
...
static size_t service_contract_len(const bccam_uart_service_entry_t *service);

static void clear_pending_mode_request(void) {
#if !defined(UNIT_TEST) && !defined(UNIT_TEST_MODE)
  taskENTER_CRITICAL();
#endif
  pending_mode_request = BCCAM_UART_MODE_REQUEST_NONE;
#if !defined(UNIT_TEST) && !defined(UNIT_TEST_MODE)
  taskEXIT_CRITICAL();
#endif
}

static void submit_mode_request(bccam_uart_mode_request_t request) {
  if (request == BCCAM_UART_MODE_REQUEST_NONE) {
    return;
  }
...
static bccam_uart_mode_request_t take_pending_mode_request(void) {
#if !defined(UNIT_TEST) && !defined(UNIT_TEST_MODE)
  taskENTER_CRITICAL();
#endif
  const bccam_uart_mode_request_t request = pending_mode_request;
  pending_mode_request = BCCAM_UART_MODE_REQUEST_NONE;
#if !defined(UNIT_TEST) && !defined(UNIT_TEST_MODE)
  taskEXIT_CRITICAL();
#endif
...
static bool reserve_rx_frame_slot_from_isr(void *context,
...
#endif
  for (uint8_t i = 0; i < BCCAM_UART_RX_FRAME_POOL_LENGTH; i++) {
    if (!rx_frame_pool[i].in_use) {
      rx_frame_pool[i].in_use = true;
      rx_frame_pool[i].length = 0;
      slot->slot = i;
      slot->bytes = rx_frame_pool[i].bytes;
      slot->capacity = sizeof(rx_frame_pool[i].bytes);
#if !defined(UNIT_TEST) && !defined(UNIT_TEST_MODE)
...
static void release_rx_frame_slot_metadata(uint8_t slot) {
  if (slot >= BCCAM_UART_RX_FRAME_POOL_LENGTH) {
    return;
  }

  rx_frame_pool[slot].in_use = false;
  rx_frame_pool[slot].length = 0;
}

static void release_rx_frame_slot(uint8_t slot) {
#if !defined(UNIT_TEST) && !defined(UNIT_TEST_MODE)
  taskENTER_CRITICAL();
#endif
  release_rx_frame_slot_metadata(slot);
#if !defined(UNIT_TEST) && !defined(UNIT_TEST_MODE)
  taskEXIT_CRITICAL();
#endif
}

static void release_rx_frame_slot_from_isr(void *context, uint8_t slot) {
  (void)context;
#if !defined(UNIT_TEST) && !defined(UNIT_TEST_MODE)
  const UBaseType_t saved_interrupt_status = taskENTER_CRITICAL_FROM_ISR();
#endif
...
static void reset_rx_collector_for_firmware_mode(void) {
  ...
  bccam_uart_rx_collector_init(&rx_collector,
                               ...
                               reserve_rx_frame_slot_from_isr,
                               release_rx_frame_slot_from_isr,
...
static const char *link_state_name(bccam_uart_link_state_t state) {
  switch (state) {
    case BCCAM_UART_LINK_UNINITIALIZED:
...
static const char *control_probe_phase_name(
  ...
  switch (phase) {
    case BCCAM_UART_CONTROL_PROBE_WAITING_FOR_DISCOVERY:
...
static const char *startup_phase_message(void) {
  ...
  switch (observation.control_probe_phase) {
    case BCCAM_UART_CONTROL_PROBE_WAITING_FOR_DISCOVERY:
      return "startup stopped while waiting for the camera deck discovery reply.";
    case BCCAM_UART_CONTROL_PROBE_WAITING_FOR_CONTROL_SERVICE:
      for (uint8_t i = 0; i < observation.advertised_service_count; i++) {
        const bccam_uart_service_entry_t *service = advertised_service_at(i);
        if (service == NULL) {
          continue;
        }
        const char control_contract[] = "bitcraze.control";
        if (service_contract_len(service) == sizeof(control_contract) - 1u &&
            memcmp(service->contract_id,
                   control_contract,
                   sizeof(control_contract) - 1u) == 0 &&
            (service->major != BCCAM_CONTROL_SERVICE_MAJOR ||
             service->minor != BCCAM_CONTROL_SERVICE_MINOR)) {
          return "startup stopped because the advertised Control service version is not supported.";
        }
      }
...
static void start_firmware_discovery_at(TickType_t now) {
#if !defined(UNIT_TEST) && !defined(UNIT_TEST_MODE)
  disable_firmware_rx_callbacks();
#endif
  reset_rx_queue_state();
  ...
  if (!bccam_firmware_uart_client_enter(&firmware_client, now)) {
    DEBUG_PRINT("UART link discovery start failed\n");
    enter_fw_resetting();
    return;
  }
...
#endif
  reset_control_probe_logging();
  service_state = BCCAM_UART_SERVICE_STATE_FW_DISCOVERY;
...
#if !defined(UNIT_TEST) && !defined(UNIT_TEST_MODE)
static void start_firmware_discovery(void) {
  start_firmware_discovery_at(xTaskGetTickCount());
}
#endif
...
static bool handle_pending_mode_request(void) {
  const bccam_uart_mode_request_t request = take_pending_mode_request();

  switch (request) {
    case BCCAM_UART_MODE_REQUEST_ENTER_BOOTLOADER:
      (void)handle_enter_bootloader_request();
      update_service_status_cache();
      return true;

    case BCCAM_UART_MODE_REQUEST_RESET_TO_FIRMWARE:
      (void)handle_reset_to_firmware_request();
      update_service_status_cache();
      return true;

    case BCCAM_UART_MODE_REQUEST_NONE:
      break;
  }
...
static bool submit_sync_request(bccam_uart_request_t *request) {
  ...
  request->result_out = &request->result;

  bccam_uart_request_t queued_request = *request;
  handle_request(&queued_request);
  update_service_status_cache();
...
static void populate_startup_report(
  ...
  memset(&report->first_advertised_service,
         0,
         sizeof(report->first_advertised_service));
  const bccam_uart_service_entry_t *first_advertised_service =
    advertised_service_at(0);
  if (first_advertised_service != NULL) {
    report->first_advertised_service = *first_advertised_service;
  }
}
#endif
...
static size_t service_contract_len(const bccam_uart_service_entry_t *service) {
  ...
  while (len < BCCAM_UART_SERVICE_CONTRACT_ID_LEN &&
         service->contract_id[len] != '\0') {
    len++;
  }
...
static void report_advertised_services(void) {
  ...
  if (observation.advertised_service_count == 0) {
    DEBUG_PRINT("bcCam UART: advertised services: none\n");
    return;
  }
  ...
  for (uint8_t i = 0; i < observation.advertised_service_count; i++) {
    ...
    const size_t contract_len = service_contract_len(service);
    char contract[BCCAM_UART_SERVICE_CONTRACT_ID_LEN + 1u] = { 0 };

    memcpy(contract, service->contract_id, contract_len);
    DEBUG_PRINT("bcCam UART: advertised service %u: id=%u contract=%s version=%u.%u\n",
                i,
                service->service_id,
                contract,
                service->major,
...
static bool handle_rx_fault_event(bccam_uart_rx_fault_t fault) {
  if (!is_rx_firmware_state(service_state)) {
    return false;
  }

  const int result =
    bccam_firmware_uart_client_on_rx_fault(&firmware_client, fault);
  if (result != BCCAM_UART_OK ||
...
static bool handle_rx_raw_frame_event(
  const bccam_uart_rx_raw_frame_t *raw_frame) {
  if (raw_frame == NULL) {
    return false;
  }

  if (!is_rx_firmware_state(service_state)) {
    release_rx_frame_slot(raw_frame->slot);
    return false;
  }

  if (raw_frame->slot >= BCCAM_UART_RX_FRAME_POOL_LENGTH ||
      raw_frame->length > BCCAM_UART_FRAME_MAX_ENCODED_SIZE) {
    release_rx_frame_slot(raw_frame->slot);
    return handle_rx_fault_event(BCCAM_UART_RX_FAULT_COLLECTOR_STATE);
  }

  rx_frame_pool[raw_frame->slot].length = raw_frame->length;
  const int result =
    bccam_firmware_uart_client_on_raw_frame(
      &firmware_client,
      rx_frame_pool[raw_frame->slot].bytes,
      raw_frame->length);
  release_rx_frame_slot(raw_frame->slot);
...
static bool handle_rx_event(const bccam_uart_rx_event_t *event) {
  ...
  switch (event->type) {
    case BCCAM_UART_RX_EVENT_FAULT:
      return handle_rx_fault_event(event->fault);

    case BCCAM_UART_RX_EVENT_RAW_FRAME:
      return handle_rx_raw_frame_event(&event->raw_frame);
...
static void handle_dequeued_rx_event(const bccam_uart_rx_event_t *event,
                                     bool *handled_any) {
  const bool handled = handle_rx_event(event);
  if (handled_any != NULL) {
...
static bool drain_rx_events(uint32_t now_ticks) {
  ...
  while (drained < BCCAM_UART_RX_DRAIN_BUDGET) {
    ...
    handle_dequeued_rx_event(&event, &handled_any);
    drained++;
...
static void service_poll_once_at(uint32_t now_ticks) {
  if (service_state == BCCAM_UART_SERVICE_STATE_FW_RESETTING) {
    start_firmware_discovery_at((TickType_t)now_ticks);
    if (service_state == BCCAM_UART_SERVICE_STATE_FW_DISCOVERY) {
      poll_firmware_link_at(now_ticks);
    }
  ...
  } else if (service_state == BCCAM_UART_SERVICE_STATE_FW_DISCOVERY ||
             service_state == BCCAM_UART_SERVICE_STATE_FW_ACTIVE) {
    poll_firmware_link_at(now_ticks);
  }
}
...
static void bcCamUartTask(void *arg) {
  ...
  while (true) {
    ...
    if (handle_pending_mode_request()) {
      did_work = true;
    }
...
void bccam_uart_service_request_bootloader(void) {
  submit_mode_request(BCCAM_UART_MODE_REQUEST_ENTER_BOOTLOADER);
}

void bccam_uart_service_request_firmware(void) {
  submit_mode_request(BCCAM_UART_MODE_REQUEST_RESET_TO_FIRMWARE);
}
...
void bccam_uart_service_test_reset(void) {
  ...
  test_firmware_control_probe_phase =
    BCCAM_UART_CONTROL_PROBE_WAITING_FOR_DISCOVERY;
  memset(test_firmware_advertised_services,
         0,
         sizeof(test_firmware_advertised_services));
  test_firmware_advertised_service_count = 0;
...
void bccam_uart_service_test_set_firmware_advertised_services(
  ...
  const uint8_t copy_count =
    (service_count <= BCCAM_UART_MAX_DISCOVERED_SERVICES) ?
    service_count : BCCAM_UART_MAX_DISCOVERED_SERVICES;
  memcpy(test_firmware_advertised_services,
         services,
         (size_t)copy_count * sizeof(test_firmware_advertised_services[0]));
  test_firmware_advertised_service_count = copy_count;
}
...
static bool handle_rx_frame_bytes(const uint8_t *bytes, uint16_t length) {
  if (bytes == NULL || !is_rx_firmware_state(service_state)) {
    return false;
  }

  const int result =
    bccam_firmware_uart_client_on_raw_frame(&firmware_client, bytes, length);
  if (result != BCCAM_UART_OK ||
      bccam_uart_runtime_get_state(&firmware_client.runtime) ==
        BCCAM_UART_LINK_FAULT) {
    enter_fw_resetting();
...
void bccam_uart_service_test_handle_rx_frame(const uint8_t *bytes,
                                             uint16_t length) {
  if (handle_rx_frame_bytes(bytes, length)) {
    poll_after_rx_event_drain_at(0);
  }
}

bool bccam_uart_service_test_handle_rx_frame_via_pool(const uint8_t *bytes,
                                                      uint16_t length,
                                                      uint8_t *slot_out) {
  bccam_uart_rx_slot_t slot = {
  ...
  if (slot_out != NULL) {
    *slot_out = BCCAM_UART_RX_SLOT_INVALID;
  }

  if (bytes == NULL ||
      !reserve_rx_frame_slot_from_isr(NULL, &slot) ||
      length > slot.capacity) {
    if (slot.slot != BCCAM_UART_RX_SLOT_INVALID) {
      release_rx_frame_slot(slot.slot);
    }
    return false;
  }
...
bool bccam_uart_service_test_handle_rx_frame_via_pool_then_queue_overflow(
  const uint8_t *bytes,
  uint16_t length,
  uint8_t *slot_out) {
  bccam_uart_rx_slot_t slot = {
  ...
  bool handled_any = false;
  const bccam_uart_rx_event_t frame_event = {
    .type = BCCAM_UART_RX_EVENT_RAW_FRAME,
    .raw_frame = {
      .slot = slot.slot,
      .length = length,
  ...
  handle_dequeued_rx_event(&frame_event, &handled_any);

  const bccam_uart_rx_event_t overflow_event = {
    .type = BCCAM_UART_RX_EVENT_FAULT,
    .fault = BCCAM_UART_RX_FAULT_QUEUE_OVERFLOW,
  };
  handled_any = handle_rx_event(&overflow_event) || handled_any;
...
void bccam_uart_service_test_handle_rx_frames(const uint8_t *first_bytes,
                                              uint16_t first_length,
                                              const uint8_t *second_bytes,
                                              uint16_t second_length) {
  bool handled_any = false;

  handled_any = handle_rx_frame_bytes(first_bytes, first_length) || handled_any;
  handled_any =
    handle_rx_frame_bytes(second_bytes, second_length) || handled_any;
...
bool bccam_uart_service_test_reserve_rx_frame_slot(uint8_t *slot_out) {
  bccam_uart_rx_slot_t slot = {
    .slot = BCCAM_UART_RX_SLOT_INVALID,
  };
  const bool reserved = reserve_rx_frame_slot_from_isr(NULL, &slot);

  if (slot_out != NULL) {
    *slot_out = reserved ? slot.slot : BCCAM_UART_RX_SLOT_INVALID;
  }
  return reserved;
}

bool bccam_uart_service_test_rx_frame_slot_in_use(uint8_t slot) {
  if (slot >= BCCAM_UART_RX_FRAME_POOL_LENGTH) {
    return false;
  }

  return rx_frame_pool[slot].in_use;
}

void bccam_uart_service_test_reset_rx_queue_state(void) {
  reset_rx_queue_state();
}
...
bool bccam_uart_service_test_handle_pending_mode_request(void) {
  return handle_pending_mode_request();
}

void bccam_uart_service_test_start_firmware_discovery(void) {
  bccam_firmware_uart_client_init(&firmware_client, &deck_controller);
  start_firmware_discovery_at(0);
}
