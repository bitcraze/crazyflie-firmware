...
#define BCCAM_FW_DISCOVERY_RETRY_MS 1000
...
static void firmware_uart_send(uint32_t size, const uint8_t *data) {
  bccam_firmware_uart_client_test_trace_entry_t *entry =
...
static void send_firmware_frame(void *context,
                                const uint8_t *data,
                                uint32_t length) {
  (void)context;
  firmware_uart_send(length, data);
}

static bool tick_has_reached(uint32_t now, uint32_t deadline) {
  return (int32_t)(now - deadline) >= 0;
}

static void schedule_discovery_retry(bccam_firmware_uart_client_t *client,
                                     uint32_t now_ticks) {
  client->next_discovery_tick = now_ticks + M2T(BCCAM_FW_DISCOVERY_RETRY_MS);
}
...
#if defined(UNIT_TEST) || defined(UNIT_TEST_MODE)
  test_trace_append_entry(BCCAM_FIRMWARE_UART_CLIENT_TEST_RUNTIME_START_DISCOVERY);
#endif
  client->next_discovery_tick = 0;
...
int bccam_firmware_uart_client_on_rx_fault(
  bccam_firmware_uart_client_t *client,
  ...
  return bccam_uart_runtime_on_rx_fault(&client->runtime, fault);
}

int bccam_firmware_uart_client_on_raw_frame(
  bccam_firmware_uart_client_t *client,
  const uint8_t *bytes,
  uint16_t length) {
  if (client == NULL ||
      bytes == NULL ||
      length > BCCAM_UART_FRAME_MAX_ENCODED_SIZE) {
    return BCCAM_UART_ERR_BAD_ARGUMENT;
  }

  return bccam_uart_runtime_on_raw_frame(&client->runtime, bytes, length);
}
...
int bccam_firmware_uart_client_on_rx_event(
  ...
  switch (event->type) {
    case BCCAM_UART_RX_EVENT_FAULT:
      return bccam_firmware_uart_client_on_rx_fault(client, event->fault);
...
int bccam_firmware_uart_client_poll(
  ...
  const uint16_t tx_flushes_before = client->runtime.tx_flushes;
  int result = bccam_firmware_uart_client_pump_tx(client);
  if (result != BCCAM_UART_OK) {
    return result;
  }

  if (client->runtime.tx_flushes != tx_flushes_before &&
      bccam_uart_runtime_get_state(&client->runtime) != BCCAM_UART_LINK_ACTIVE) {
    schedule_discovery_retry(client, now_ticks);
  }
  ...
  if (bccam_uart_runtime_get_state(&client->runtime) == BCCAM_UART_LINK_UNINITIALIZED &&
      client->next_discovery_tick != 0 &&
      tick_has_reached(now_ticks, client->next_discovery_tick)) {
    result = bccam_uart_runtime_retry_discovery(&client->runtime);
    if (result != BCCAM_UART_OK) {
      schedule_discovery_retry(client, now_ticks);
      return result;
    }
  ...
  if (bccam_uart_runtime_get_state(&client->runtime) == BCCAM_UART_LINK_ACTIVE) {
    result = bccam_uart_runtime_step_control_probe(&client->runtime);
    if (result != BCCAM_UART_OK) {
      return result;
    }

    result = bccam_firmware_uart_client_pump_tx(client);
    if (result != BCCAM_UART_OK) {
      return result;
    }
  }
...
const bccam_uart_service_entry_t *
bccam_firmware_uart_client_advertised_service(
  const bccam_firmware_uart_client_t *client,
  uint8_t index) {
  if (client == NULL || index >= client->runtime.link.service_count) {
    return NULL;
  }

  return &client->runtime.link.services[index];
}
