...
%inline %{
// --- Lighthouse sweep angle replay -----------------------------------------
// sweepAngleMeasurement_t holds the deck sensor position, the base station
// geometry and the calibration as pointers, and the measurement model as a
// function pointer. SWIG cannot fill those from Python, so build them here.
// The caller owns what make_* returns and should keep it alive for as long as
// the measurement is used; the emulator caches one per sensor / base station.
