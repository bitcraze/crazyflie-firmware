...
echo ">> Checking for dropped uSD events (usd.eventsRequested vs usd.eventsAccepted)..."
...
DROP_LINE="$("$CFCLI" -u "$URI" --timeout 3000 --csv log print usd.eventsRequested,usd.eventsAccepted 2>/dev/null | tail -n 1 || true)"
EVT_REQ=""
EVT_ACC=""
if [[ -n "$DROP_LINE" ]]; then
  EVT_REQ="$(echo "$DROP_LINE" | awk -F',' '{print $(NF-1)}' || true)"
  EVT_ACC="$(echo "$DROP_LINE" | awk -F',' '{print $NF}' || true)"
fi

if [[ ! "$EVT_REQ" =~ ^[0-9]+$ || ! "$EVT_ACC" =~ ^[0-9]+$ ]]; then
  echo "!! WARNING: could not read usd.eventsRequested/usd.eventsAccepted." >&2
  echo "   Firmware without the drop counters? Losslessness NOT verified." >&2
elif [[ "$EVT_REQ" != "$EVT_ACC" ]]; then
  echo "!! DROPPED EVENTS: eventsRequested=$EVT_REQ eventsAccepted=$EVT_ACC ($((EVT_REQ - EVT_ACC)) lost)." >&2
  ...
  echo ">> OK: no dropped events (eventsRequested = eventsAccepted = $EVT_REQ)."
fi
